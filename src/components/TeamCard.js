...
export default function TeamCard({ member }) {
  return (
    <div className="group relative bg-white/[0.02] backdrop-blur-md border border-[#1D63B8]/30 hover:border-[#1D63B8] rounded-2xl p-6 flex flex-col items-center text-center transition-all duration-300 hover:-translate-y-1.5 shadow-2xl overflow-hidden">
      
      {/* Member Photo */}
      <div className="relative w-32 h-32 rounded-full overflow-hidden mb-4 border-2 border-cyan-500/30 bg-slate-950/40 flex items-center justify-center">
      ...
      {member.areasOfInterest && member.areasOfInterest.length > 0 && (
        <div className="flex flex-wrap justify-center gap-1.5 mb-4">
          {member.areasOfInterest.map((area, idx) => (
            <span key={idx} className="text-[10px] bg-slate-950/60 text-slate-300 px-2.5 py-0.5 rounded-full border border-slate-800">
              {area}
            </span>
          ))}
        </div>
